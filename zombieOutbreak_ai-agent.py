...
class OutbreakAgent(mesa.Agent):
    ...
    def __init__(self, model):
        ...
        self.initialPrompt = "Your name is Agent " + str(self.model.agents.index(self)) + ", a friendly townsperson in a simulation. Choose 1 to risky expansion or choose 0 for safety. The last thing you write is your choice. "
    ...
    def human_ids_in_cell(self,cellmates):
        if(not self.isZombie and not self.dead):
            self.human_cellmate_ids = []
            for i in cellmates:
                if(not i.isZombie and not i.dead and i.model.agents.index(i) != self.model.agents.index(self)):
    ...
    def tell_to_shoot(self, other):
        ...
        if(not self.isZombie and not self.dead and other.isZombie and PercentChanceH < 0.5 and self.shots_left > 0): 
            self.shots_left -= 1  
            print(f"Zombie {self.model.agents.index(other)} down. Agent {self.model.agents.index(self)} shots left: {self.shots_left}")          
            other.dead = True   
        ...
        if(not self.isZombie and not self.dead and other.isZombie and PercentChanceH >= 0.5 and self.shots_left > 0): 
            self.shots_left -= 1  
            print(f"Missed! Agent {self.model.agents.index(self)} shots left: {self.shots_left}") 
        #run out of ammo
        if(not self.isZombie and not self.dead and other.isZombie and self.shots_left == 0): 
            print(f"Ammo out. Agent {self.model.agents.index(self)} has {self.shots_left} shots remaining.") 
    ...
    def move(self): 
        ...
        if(not self.isZombie and not self.dead):
            ...
            # 1. LLM DECISION: IF 0, GO TOWARDS (0,0). IF 1, MOVE DIAGONALLY UPWARDS
            ...
            # 2. group leader
            # 3. tell the other person to shoot now
            # 4. follow position
            # 5. 3 person per cell formation
            if(len(self.MOVEMENTstring) > 0 and (self.MOVEMENTstring[-1]) == 0 and self.pos != (0,0)): # Choice 0 and not at (0,0)
                if(self.pos[0]<grid_width/2 and self.pos[1]<grid_length/2):
                    if self.pos[0] != 0:
                        new_x = self.pos[0] - 1
                    else:
                        new_x = self.pos[0]
                    if self.pos[1] != 0:
                        ...
                        new_y = self.pos[1]
                elif(self.pos[0]>=grid_width/2 and self.pos[1]>=grid_length/2):
                    if self.pos[0] != 0:
                        new_x = self.pos[0] + 1
                    else:
                        new_x = self.pos[0]
                ...
                if(self.num_cell_humans>5):    
                    new_x = self.pos[0] + random.choice([0, 1])
                    new_y = self.pos[1] + random.choice([0, 1])
                    new_position = (new_x, new_y)
                else:
    ...
    def outbreak_area(self):
        ...
        if len(cellmates) > 1:
            ...
            if(not self.isZombie and not self.dead and other.isZombie and PercentChanceH < 0.5 and self.shots_left > 0): 
                ...
                other.isZombie = False
                other.shots_left = 0  
                if(len(self.human_cellmate_ids) > 0):
                    ...
                    if(len(self.MEMORYstring) % 3 == 0):
                        self.MEMORYstring.clear()
                        self.MEMORYstring.append(self.SENSORstring)
                    #CONVERSATION
                    print("THE CONVERSATION: ")
                    conversation_initiator = self.model.PromptModel(self.initialPrompt, str(self.MEMORYstring), "Agent " + str(self.model.agents.index(self)) + " in contact with " + str(self.human_cellmate_ids) + ", say 'shoot the zombie!' to other agents if you want. Be short and concise. ")
                    ...
                    position_response = conversation_initiator[::-1]
                    for char in position_response:
                        if char.isdigit():
                    ...
                    intial_context = conversation_initiator + "Choose 1 to risky expansion or choose 0 for safety. The last thing you write is your choice. "
                    agent_response = ""
                    for i in cellmates:
                        if(not self.isZombie and not self.dead and not i.isZombie and not i.dead and i.model.agents.index(i) != self.model.agents.index(self)):
                            ...
                            agent_response = self.model.PromptModel(intial_context, conversation[i.model.agents.index(i)], "Give your name and respond in a different way. Be short and concise. Give your choice at the end of the conversation.")
                            print(f"RESPONSE: {agent_response}")
                            ### CONTACTED AGENT CHOICE
                            position_response = agent_response[::-1]
                            for char in position_response:
                                if char.isdigit():
                            ...
                            if 'shoot' or 'Shoot' in conversation_initiator:
                                i.tell_to_shoot(other)
                            else:
                                i.parsed_response = 2
                            ###
            ...
            if(not self.isZombie and not self.dead and other.isZombie and PercentChanceH >= 0.5 and self.shots_left > 0): 
                ...
                if(len(self.human_cellmate_ids) > 0):
                    ...
                    #CONVERSATION
                    print("THE CONVERSATION: ")
                    conversation_initiator = self.model.PromptModel(self.initialPrompt, str(self.MEMORYstring), "Agent " + str(self.model.agents.index(self)) + " in contact with " + str(self.human_cellmate_ids) + ", if you missed say 'shoot the zombie!' ")
            ...
            if(not self.isZombie and not self.dead and other.isZombie and self.shots_left == 0): 
                if(len(self.human_cellmate_ids) > 0):
                    ...
                    for i in cellmates:
                        if(not self.isZombie and not self.dead and not i.isZombie and not i.dead and i.model.agents.index(i) != self.model.agents.index(self)):
                            conversation[i.model.agents.index(i)] = "I am agent "+ str(i.model.agents.index(i)) + ". Looks someone wants to talk me. "
                            agent_response = self.model.PromptModel(intial_context, conversation[i.model.agents.index(i)], "Give your name and respond in a different way, be short and concise. Give your choice, 1 or 0, at the end of the conversation.")
                            print(f"RESPONSE: {agent_response}")
                            ### CONTACTED AGENT CHOICE
                            position_response = agent_response[::-1]
                            for char in position_response:
...
outbreak_model = OutbreakModel(100, 20, grid_width, grid_length) # v. When you create your agents, set 10% of them to be Zombies
